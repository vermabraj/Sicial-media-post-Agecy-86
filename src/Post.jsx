...
function TestimonialCard(props) {
  ...
  const { user, content, _id, likes, index } = props;

  let token = localStorage.getItem("token");
  let post_Id = localStorage.getItem("_id");
  const updateLike = (postId) => {
    setIsLiking(true);
    axios
      .put(
        `https://agency-backend-e2v8.onrender.com/posts/likes/${postId}`,
        {},
        {
          headers: {
            Authorization: token,
  ...
  return (
    <Box>
      ...
      <Button
        variant="ghost"
        isLoading={isLiking}
        color={likes?.includes(user?._id) ? "blue.400" : ""}
        loadingText="Like"
